...
export class UploadService {
    ...
    async uploadToStorage(storage: Storage, req: Request) {
        ...
        switch (storage.type.name) {
            ...
            case 'backblaze':
                ...
                break;
            case 'ftp':
                return this.uploadWithFTP(storage, req);
                break;
    ...
    async uploadWithFTP(storage, req) {
        const parts = await FormHelper.getBuffers(req);
        const files = await this.uploadPartsToFTP(storage, parts);
        return files;
    }
    ...
    uploadPartsToFTP(storage, parts) {
        const files = parts.map(part => this.uploadPartToFTP(storage, part));
        return Promise.all<Upload>(files);
    }
    ...
    async uploadPartToFTP(storage, upload){
        const ftpClient = new Client();
        const customConfig = storage.config.custom;
        const fileName = upload.nameOverride || uuid() + '.' + upload.type.extension;
