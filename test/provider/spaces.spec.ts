...
describe('Upload to spaces (DigitalOcean)',
    function () {
        this.timeout(100000);
        const uuid = v4();
        ...
        before(async () => {
            storage = await ProviderService.instance.findByName('my_spaces_files');
        });

        it('Should upload '+ nameOverride +' to spaces (DigitalOcean)', async function () {
        ...
        it('Should download '+ nameOverride +' from spaces (DigitalOcean)', async function () {
            const response = await UploadServiceTest.instance.download(storage, nameOverride);
        });

        it('Should delete '+ nameOverride +' from Spaces', async function () {
